# Антон Балмаков

**Python Developer | Automation | System Analysis**

---

## Обо мне

Я системный аналитик с большим опытом автоматизации бизнес-процессов и разработки IT-решений.  
В своей работе совмещаю аналитическое мышление и разработку инструментов на Python для автоматизации задач, интеграций и обработки данных.

Более 17 лет работал в электроэнергетике, где занимался проектированием и внедрением автоматизированных систем, интеграциями между платформами и оптимизацией процессов.

Сейчас развиваюсь в направлении Python-разработки, автоматизации и AI-assisted development.

---

## Технологии и инструменты

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Django](https://img.shields.io/badge/Django-0C4B33?style=for-the-badge&logo=django&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white)
![REST API](https://img.shields.io/badge/REST_API-005571?style=for-the-badge)
![JSON](https://img.shields.io/badge/JSON-000000?style=for-the-badge&logo=json&logoColor=white)
![XML](https://img.shields.io/badge/XML-FF6600?style=for-the-badge)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![BPMN](https://img.shields.io/badge/BPMN-1E90FF?style=for-the-badge)
![UML](https://img.shields.io/badge/UML-5A5A5A?style=for-the-badge)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)
![VBA](https://img.shields.io/badge/VBA-867DB1?style=for-the-badge)

---

## GitHub статистика

![Anton GitHub stats](https://github-readme-stats.vercel.app/api?username=anton-balmakov&show_icons=true&theme=default)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=anton-balmakov&layout=compact)
---

## Проекты

Раздел находится в процессе наполнения.  
Здесь размещаются проекты на Python, связанные с автоматизацией, обработкой данных, Telegram-ботами и веб-разработкой.

### Закреплённые репозитории
- **Diplom_TaskMentor** — дипломный Django-проект для управления задачами и прогрессом учеников
- **TelegramBots-Aiogram** — набор Telegram-ботов на Python с использованием aiogram
- **Python-LearningLab** — учебные и практические проекты: ООП, парсинг, Flask, API, анализ данных

---

## Образование

- **Системный аналитик** — Яндекс Практикум  
- **Программист на Python с помощью ChatGPT** — Zerocoder University  

---

## Контакты

**Email:** anton_bal_23@mail.ru  
**Telegram:** [@tonas_347](https://t.me/tonas_347)
